...
from mangled_names import ConstraintsNames, FiltersNames
...
def initializeIpoptProblem(TOP, previousHistory=[], recording = True):
    ...
    constraintsNumber = 1
    varsNumber = TOP.getDensities().shape[0]
    
    # Variables bounds (\in [0, 1])
    lb = np.zeros(varsNumber)
    ub = np.ones(varsNumber)
    ...
    return nlp, problemObj  # nlp runs optimization, optimization history stores intermediate data
